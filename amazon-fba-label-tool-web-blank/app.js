...
function makeRecordFromSingle() {
  return {
    ...
    condition: els.conditionText.value.trim(),
    origin: els.originText.value.trim(),
...
function makeRecordsFromBatch() {
  const condition = els.batchConditionText.value.trim();
  const origin = els.batchOriginText.value.trim();
